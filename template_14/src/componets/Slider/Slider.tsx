   ...
   useEffect(() => {
      const handleResize = () => {
         ...
         if (typeof window !== 'undefined') {
            const newMaxVisibleSlides = isChangeVisibleSlide && window?.innerWidth < 600 ? 1 : 2;
   ...
   return (
      <div className={styles.sliderContainer}>
         {currentIndex > 0 ? (
            ...
            <Image src={arrow_gray} alt="Previous" layout='responsive' />
         </button>}
         ...
         {currentIndex < totalSlides - maxVisibleSlides ? (
            <button className={styles.slideButton} onClick={handleNextClick}>
               <Image src={arrow} alt="Next" layout='responsive' />
            </button>
         ) : <button className={styles.slideButton} >
            <Image src={arrow_gray} alt="Next" layout='responsive' />
         </button>}
      </div>
   );
};
